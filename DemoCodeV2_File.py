...
class SFdatabase:
    ...
    def readDB(self, sm_records='', pr_records='', state=''):
        ...
        self.pr_df = self.cur.execute(
            f"SELECT {pr_records} PROVIDER.PROVIDERID, FIRSTNAME, LASTNAME, PROVIDERADDRESSID FROM PROVIDER \
                ...
                WHERE State='NY' AND PROVIDERTYPE = 1 AND PROVIDER.PROVIDERID NOT IN (SELECT PROVIDERID FROM PROVIDERSTANDARDMAPPER_02)"
            ).fetch_pandas_all()
    ...
    def readFile(self):
        ...
        parquet_file="PRData_NY.parquet"
        print(parquet_file)
        self.sm_df=pd.read_parquet(parquet_file,engine="pyarrow")
        ...
        # Provider registry
        print('Reading Parket file::')
        parquet_file="SMData_NY.parquet"
        print(parquet_file)
        self.pr_df=pd.read_parquet(parquet_file,engine="pyarrow")
        print('File Read below data found')
    ...
    def Rapid_Merge(self, df_1, df_2, key1, key2, threshold=90, limit=1):
        """
        :param df_1: the left table to join
        :param df_2: the right table to join
        :param key1: key column of the left table
    ...
    def matchNames(self):
        ...
        self.final_df=self.Rapid_Merge(self.sm_df, self.pr_df, 'FULLNAME', 'FULLNAME', 80)
    ...
    def toSnowflake(self):
        ...
        to_snowflake(dd.from_pandas(data=self.final_df, npartitions=1), 'providerstandardmapper_02', self.secret_keys)
